...
// TODO:
// Deadline - December 15 for submission
// Include ASCII art at the end of the file or maybe a hyperlink somewhere
// Cool things to have:
// -- When pokemon uses rest, increase HP by x amount
// -- Missing "It's not very effective! when attack is weak against opponent


// Struct for the moves, will need to create one for each Pokemon. Think about a better way to scale this. Read text files and populate?
typedef struct {
    char move[100];
    int power;
...
int main()
{
    // Initialize choice and pokemon structs
    srand(time(NULL));
